...
def ensure_start_date_eq_bottom(top_df, bot_df):
    ...
    if proj_start_date != bot_start_date:
        raise ValidationError(
            f"Projected start date {proj_start_date} and table start date {bot_start_date} don't match.")
...
def ensure_min_date_eq_bottom(top_df, bot_df):
    ...
    if proj_min_bal != bot_min_bal:
        raise ValidationError(f"Projected min balance {proj_min_bal} and table min balance {bot_min_bal} don't match.")
    if proj_min_date != bot_min_date:
        raise ValidationError(f"Projected min date {proj_min_date} and table min date {bot_min_date} don't match.")
...
def ensure_escrow_pay_eq_bottom(top_df, bot_df):
    ...
    if to_amount.where(to_amount != escrow_amount).any():
        raise ValidationError('A value in To Amount does not equal the expected monthly escrow amount.')
